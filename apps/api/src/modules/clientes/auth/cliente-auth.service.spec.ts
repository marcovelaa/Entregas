...
function buildCliente(
  overrides: Partial<ClienteConCredenciales> = {},
): ClienteConCredenciales {
  return {
    id: '1',
    nombres: 'Ana',
...
describe('ClienteAuthService.validarCredenciales', () => {
  it('rechaza con UnauthorizedException si el email no existe', async () => {
    ...
    await expect(
      service.validarCredenciales('nadie@example.test', 'x'),
  ...
  it('rechaza con UnauthorizedException si la contraseña no coincide', async () => {
    ...
    clienteRepo.buscarPorEmailConCredenciales.mockResolvedValue(
      buildCliente({ passwordHash: hash }),
    );

    await expect(
      service.validarCredenciales('ana@example.test', 'incorrecta'),
    ).rejects.toThrow(UnauthorizedException);
  });
  ...
  it('acepta credenciales correctas', async () => {
    ...
    const cliente = await service.validarCredenciales(
      'ana@example.test',
      'correcta',
    );
...
describe('ClienteAuthService.solicitarRecuperacion', () => {
  it('no revela si el email existe: siempre resuelve sin lanzar', async () => {
    ...
    await expect(
      service.solicitarRecuperacion('nadie@example.test'),
    ).resolves.toEqual({});
  });
...
describe('ClienteAuthService.restablecerPassword', () => {
  ...
  it('rechaza con BadRequestException si el token ya fue usado', async () => {
    ...
    await expect(
      service.restablecerPassword('token', 'nuevaPassword123'),
    ).rejects.toThrow(BadRequestException);
  });
  ...
  it('actualiza el password y marca el token como usado cuando es válido', async () => {
    ...
    expect(clienteRepo.actualizarPassword).toHaveBeenCalledWith(
      '1',
      expect.any(String),
    );
...
describe('ClienteAuthService.cambiarPassword', () => {
  it('rechaza con UnauthorizedException si la contraseña actual es incorrecta', async () => {
    ...
    clienteRepo.obtenerConCredencialesPorId.mockResolvedValue(
      buildCliente({ passwordHash: hash }),
    );

    await expect(
      service.cambiarPassword('1', 'actual-incorrecta', 'nuevaPassword123'),
    ).rejects.toThrow(UnauthorizedException);
  });
