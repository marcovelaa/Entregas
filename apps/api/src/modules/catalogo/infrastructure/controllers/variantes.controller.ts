...
import { CrearVarianteDto, ActualizarVarianteDto, CrearVariantesBulkDto } from '../../application/dtos/variante.dto';
...
export class VariantesController {
  ...
  async listarPorProducto(@Param('productoId', ParseBigIntPipe) productoId: bigint) {
    const variantes = await this.listarVariantesUseCase.execute(productoId.toString());
    return variantes.map(serializeVariante);
  }

  @Patch(':id')
  async actualizar(@Param('id', ParseBigIntPipe) id: bigint, @Body() dto: ActualizarVarianteDto) {
  ...
  @UseInterceptors(FileInterceptor('image', {
    ...
    fileFilter: (req, file, cb) => {
      if (!file.mimetype.match(/^image\/(jpeg|png|webp|gif)$/)) {
        cb(new BadRequestException('Solo imágenes JPG, PNG, WebP o GIF') as any, false);
        return;
      }
      cb(null, true);
    },
  ...
  async uploadImagen(
    ...
    const v = await this.actualizarVarianteUseCase.execute(id, { imagen_url: imageUrl } as any);
    return serializeVariante(v);
  }
}
