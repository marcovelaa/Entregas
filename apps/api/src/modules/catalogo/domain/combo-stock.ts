...
export function stockDisponibleDeComponente(componente: any): number {
  ...
  const inventario =
    componente.variante_id != null
      ? componente.variante?.Inventario
      : componente.componente_producto?.Inventario;
  if (Array.isArray(inventario) && inventario.length > 0) {
    return inventario.reduce(
      (acc: number, f: any) =>
        acc + Math.max(0, (f.cantidad_disponible ?? 0) - (f.reservado ?? 0)),
      0,
    );
  }
  return typeof componente.stock_disponible === 'number'
    ? Math.max(0, componente.stock_disponible)
    : 0;
}

export function computeStockBom(
  componentes: Array<BomComponenteInput | null | undefined>,
): number {
  ...
  return computeStockBom(
    componentes.map((c) => {
      ...
      const matching = filas.filter(
        (f) =>
          f.producto_id === id &&
          (varianteId !== null ? (f.variante_id ?? null) === varianteId : true),
      );
