describe('Composite Products (Combos con Receta) - Logic & Stock Calculation', () => {
  it('calculates dynamic virtual stock for a combo based on minimum available component ratio', () => {
    ...
    const comboRecipe = [
      { componenteId: '1', nombre: 'Mochila', cantidadRequerida: 1, stockFisico: 10 },
      { componenteId: '2', nombre: 'Cuaderno', cantidadRequerida: 2, stockFisico: 30 }, // 30 / 2 = 15 kits
      { componenteId: '3', nombre: 'Bolígrafo', cantidadRequerida: 3, stockFisico: 18 }, // 18 / 3 = 6 kits (bottleneck)
    ];

    const calculateVirtualComboStock = (components: typeof comboRecipe) => {
      if (!components || components.length === 0) return 0;
      return Math.min(...components.map((c) => Math.floor(c.stockFisico / c.cantidadRequerida)));
    };
  ...
  it('returns 0 available kits if any required component is completely out of stock', () => {
    const comboRecipe = [
      { componenteId: '1', nombre: 'Mochila', cantidadRequerida: 1, stockFisico: 10 },
      { componenteId: '2', nombre: 'Cuaderno', cantidadRequerida: 2, stockFisico: 0 }, // 0 stock
    ];
  ...
  it('calculates the package savings accurately comparing sum of components vs fixed bundle price', () => {
    ...
    const regularTotal = components.reduce((sum, c) => sum + c.precioUnitario * c.cantidad, 0); // 120 + 50 = 170
