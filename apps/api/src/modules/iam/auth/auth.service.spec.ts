...
function buildUsuario(overrides: Partial<{ activo: boolean; passwordHash: string }> = {}) {
  return new Usuario(
    1n,
    'public-id-1',
    10n,
...
describe('AuthService.validarCredenciales', () => {
  it('rechaza con UnauthorizedException si el email no existe', async () => {
    ...
    await expect(service.validarCredenciales('nadie@entregas.bo', 'x')).rejects.toThrow(UnauthorizedException);
  });
  ...
  it('rechaza con UnauthorizedException si el usuario está inactivo', async () => {
    ...
    await expect(service.validarCredenciales('ana@entregas.bo', 'x')).rejects.toThrow(UnauthorizedException);
  });

  it('rechaza con UnauthorizedException si el password no coincide', async () => {
    const { service, usuarioRepo } = createHarness();
    const hash = await bcrypt.hash('password_correcto', 10);
    usuarioRepo.findByEmail.mockResolvedValue(buildUsuario({ passwordHash: hash }));

    await expect(service.validarCredenciales('ana@entregas.bo', 'password_incorrecto')).rejects.toThrow(
  ...
  it('devuelve el usuario cuando el email existe, está activo y el password coincide', async () => {
    ...
    const result = await service.validarCredenciales('ana@entregas.bo', 'password_correcto');
...
describe('AuthService.login', () => {
  it('firma un access_token y un refresh_token, embebiendo permisos y rol del usuario', async () => {
    ...
    rolRepo.findById.mockResolvedValue(new Rol(10n, 'Vendedor', null, true, new Date(), new Date()));
    ...
    expect(result.usuario).toEqual(
      expect.objectContaining({ id: '1', email: 'ana@entregas.bo', rol: 'Vendedor' }),
    );

    const [payload, options] = jwtService.sign.mock.calls[0];
    expect(payload).toEqual(
      expect.objectContaining({ sub: '1', rolNombre: 'Vendedor', permisos: ['ventas:crear', 'ventas:ver'] }),
    );
    expect(options).toEqual(expect.objectContaining({ secret: 'test-secret', expiresIn: '8h' }));

    // Registers last access on the persisted user
    expect(usuarioRepo.update).toHaveBeenCalledWith(expect.objectContaining({ ultimoAccesoEn: expect.any(Date) }));
  });
});
...
describe('AuthService.refrescar', () => {
  it('rechaza con UnauthorizedException si el refresh token no verifica', async () => {
    ...
    await expect(service.refrescar('token-invalido')).rejects.toThrow(UnauthorizedException);
  });
  ...
  it('rechaza con UnauthorizedException si el usuario del token ya no existe o está inactivo', async () => {
    ...
    await expect(service.refrescar('token-valido')).rejects.toThrow(UnauthorizedException);
  });
