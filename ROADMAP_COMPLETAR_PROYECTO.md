...
**Hallazgos nuevos durante la verificación (sin relación con este paquete, pendientes de decidir):**
- `test/app.e2e-spec.ts` falla: `GET /` devuelve 401 en vez de 200 — el guard global de auth (sesión anterior) dejó sin excepción la ruta raíz/health-check.
- `test/catalogo.e2e-spec.ts` falla: su `afterAll` borra `variante` antes que `movimientos_inventario`, violando la FK; al no llegar a `app.close()`, cualquier cron registrado en esa corrida queda colgado (así se detectó).
