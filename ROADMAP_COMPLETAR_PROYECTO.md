...
**Cierre de 3.2:** completado y verificado (2026-08-13).
- **Modelo & Prisma:** Modelo `Bitacora` mejorado con `public_id`, `tipo_actor` (`USUARIO`, `CLIENTE`, `SISTEMA`, `WEBHOOK`), `usuario_id`, `cliente_id`, `request_id`, `ip`, `user_agent`, `entidad`, `entidad_id`, `operacion`, `datos_anteriores`, `datos_nuevos`, e índices multiyave. DB sincronizada con `prisma db push`.
- **Sanitización & Servicio:** `BitacoraService` sanitiza automáticamente de forma recursiva cualquier campo sensible (`password`, `password_hash`, `token`, `secret`, `jwt`, `access_token`, `refresh_token`, `codigo_reset`, `credit_card`) reemplazándolos por `'[REDACTED]'`.
...
**Cierre de 3.4:** completado y verificado (2026-08-14).
- **Reglas de Combos & Stock Virtual:** Integración de `computeStockBom` y `computeSellable` en `ObtenerProductoUseCase` y `ListarProductosUseCase` para calcular el stock virtual disponible de paquetes/combos según la disponibilidad de sus componentes individuales ($min(S_{comp} / Q_{comp})$), así como la validación de ventanas temporales, cupos y programación semanal (`dias_semana`).
- **Acceso Público a Catálogo:** Añadido el decorador `@Public()` a los endpoints públicos de consulta de productos (`GET /productos` y `GET /productos/:publicId`), permitiendo la navegación de catálogo e-commerce sin requerir autenticación.
- **Verificación:** 31/31 suites unitarias passed (207/207 tests); 11/11 suites e2e passed incluyendo `combos-catalogo.e2e-spec.ts` (3/3 tests) contra PostgreSQL real; `tsc --noEmit` y `next build` limpios en frontend.
